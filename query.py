...
from prompts import LOCAL_SEARCH, COMMUNITY_SUMMARY
...
class QueryEngine:
    ...
    def local_search(self, query: str, graph_path: str) -> str:
        ...
        response = self.llm.create_chat_completion(
            messages=[
                {"role": "system", "content": "Answer the query using only the provided context. Be specific and concise."},
...
if __name__ == "__main__":
    ...
    local_result = engine.local_search(
        "What did Barack Obama study at Columbia University?",
        "graphs/1_graph.pkl"
    )
    print("\nLocal Search Result:", local_result)
    
    global_result = engine.global_search(
        "What were Obama's major achievements as president?",
        doc_id=1
    )
